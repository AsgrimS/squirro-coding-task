...
@router.post("/", response_description="Add new document", response_model=Document)
async def create_document(document: DocumentInput, documents_collection=Depends(get_documents_collection)):
    ...
    created_document = await documents_collection.find_one({"_id": new_document.inserted_id})
